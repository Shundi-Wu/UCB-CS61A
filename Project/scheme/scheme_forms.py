...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 10
        "*** YOUR CODE HERE ***"
        symbol, formal, body = signature.first, signature.rest, expressions.rest
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        expression = bindings.first
        name, val_expr = expression.first, expression.rest
        validate_form(val_expr, 1, 1)
        names, vals = Pair(name, names), Pair(scheme_eval(val_expr.first, env), vals) # Pair(x, val_expr) -- val_expr = Pair(val, nil)
