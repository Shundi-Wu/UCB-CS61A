...
def pick(paragraphs, select, k):
    ...
    "*** YOUR CODE HERE ***"
    result = []
    result += [i for i in paragraphs if select(i) ==  True]
    if k >= len(result):
        return ''
    else:
        return result[k]
...
def about(subject):
    ...
    def about_subject(paragraph):
        for onesubject in subject:
            if onesubject in split(lower(remove_punctuation(paragraph))):
                return True
        return False
...
def accuracy(typed, source):
    ...
    # BEGIN PROBLEM 3
    "*** YOUR CODE HERE ***"
    lentype = len(typed_words)
    index = min(lentype, len(source_words))
    correct_count = 0 
    if lentype == 0:
    ...
    for i in range(index):
        if typed_words[i] == source_words[i]:
            correct_count += 1
    accuracy_rate = correct_count * 100 / lentype
...
def wpm(typed, elapsed):
    ...
    words_per_minute = typed_words * 60 / elapsed
...
def time_per_word(words, timestamps_per_player):
    ...
    "*** YOUR CODE HERE ***"
    times = []
    for timelist in timestamps_per_player:
        newtimelist = []
        for i in range(len(timelist)-1):
            newtimelist += [timelist[i+1]-timelist[i]]
        times += [newtimelist]
    ...
    for i in word_indices:
        ...
        for j in player_indices:
            if time(match, j, i) < shortest_time:
                ...
                fastest_player = j
        finalist[fastest_player] += [get_word(match, i)]
    return finalist
    # END PROBLEM 10
